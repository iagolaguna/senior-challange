...
public final class GrahamScan {
     ...
     */
    protected static boolean areAllCollinear(List<GrahamPoint> points) {

        if (points.size() < 2) {
            return true;
        }

        final GrahamPoint a = points.get(0);
        final GrahamPoint b = points.get(1);

        for (int i = 2; i < points.size(); i++) {

            GrahamPoint c = points.get(i);
     ...
     *             if <code>xs</code> and <code>ys</code> don't have the same size,
     *             if all points are collinear or if there are less than 3 unique
     *             points present.
     */
    public static List<GrahamPoint> getConvexHull(double[] xs, double[] ys) throws IllegalArgumentException {

        if (xs.length != ys.length) {
            throw new IllegalArgumentException("xs and ys don't have the same size");
        }

        List<GrahamPoint> points = new ArrayList<>();

        for (int i = 0; i < xs.length; i++) {
            points.add(new GrahamPoint(xs[i], ys[i]));
        }
     ...
     *             points present.
     */
    public static List<GrahamPoint> getConvexHull(List<GrahamPoint> points) throws IllegalArgumentException {

        List<GrahamPoint> sorted = new ArrayList<>(getSortedPointSet(points));
        ...
        for (int i = 2; i < sorted.size(); i++) {
            ...
            GrahamPoint head = sorted.get(i);
            GrahamPoint middle = stack.pop();
            GrahamPoint tail = stack.peek();
        ...
        return new ArrayList<GrahamPoint>(stack);
    }
     ...
     *         point exists, the one with the lowest x coordinate is returned.
     */
    protected static GrahamPoint getLowestPoint(List<GrahamPoint> points) {

        GrahamPoint lowest = points.get(0);

        for (int i = 1; i < points.size(); i++) {

            GrahamPoint temp = points.get(i);
     ...
     */
    protected static Set<GrahamPoint> getSortedPointSet(List<GrahamPoint> points) {

        final GrahamPoint lowest = getLowestPoint(points);

        TreeSet<GrahamPoint> set = new TreeSet<>(new Comparator<GrahamPoint>() {
            @Override
            public int compare(GrahamPoint a, GrahamPoint b) {
                ...
                if (thetaA < thetaB) {
                    ...
                    double distanceA = Math
                            .sqrt(((lowest.x - a.x) * (lowest.x - a.x)) + ((lowest.y - a.y) * (lowest.y - a.y)));
                    double distanceB = Math
                            .sqrt(((lowest.x - b.x) * (lowest.x - b.x)) + ((lowest.y - b.y) * (lowest.y - b.y)));
     ...
     */
    protected static Turn getTurn(GrahamPoint a, GrahamPoint b, GrahamPoint c) {
